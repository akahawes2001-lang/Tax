...
# Перечень люксовых марок/моделей (постановление №194)
LUXURY_RULES = [
    ("Audi", "A8", 3),
    ("Audi", "RS6", 3),
    ("Audi", "RS7", 4),
...
async def import_cars():
    # Создаём таблицу car_references, если её нет
    ...
    async with async_session() as session:
        ...
        for row in rows:
            try:
                ...
                mass_str = row.get("curb_weight_kg", "").strip()
                ...
                session.add(
                    CarReference(brand=brand, model=model, mass=mass, is_luxury=luxury)
                )
                count += 1
                if count % 1000 == 0:
